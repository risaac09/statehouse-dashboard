...
## Self-running
Daily GitHub Action refreshes `data/` from OpenStates. This repo is a ring-2 keep: low-touch by design, near-zero attention needed once the `OPENSTATES_API_KEY` secret is set. Don't add scheduled maintenance beyond what the Action already does.

## Working here
- `scripts/normalize.mjs` is pure and unit-tested (`node scripts/test.mjs`) — the data-cleaning logic lives there, not in the fetch step.
